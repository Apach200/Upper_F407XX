...
{
    /**
     * - SDO client may start new download to or upload from specified node, specified index and specified subindex.
     *    		 It can start normal or block communication.
...
typedef enum
{
    ...
    CO_SDO_RT_uploadDataBufferFull  = 5,    /**< Data buffer is full. SDO client: data must be read before next upload cycle begins. */
    ...
    CO_SDO_RT_blockUploadInProgress = 2,    /**< Block upload   is in progress. Receiving train of messages. SDO client: Data must not be read in this state. */
 ...
 *
 * @param OD_1200_SDOsrvPar OD entry for SDO server parameter (0x1200+),
 * 			 can be NULL for default single SDO server and must not be NULL for additional SDO servers.
 * 			 With additional SDO servers it may also have IO extension enabled,
 * 			 		to allow dynamic configuration (see also @ref CO_CONFIG_FLAG_OD_DYNAMIC).
 *
 * @param nodeId If this is first SDO channel, then "nodeId" is CANopen Node ID of this device.
 * 				In all additional channels "nodeId" is ignored.
 *
