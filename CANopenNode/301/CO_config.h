 ...
 *
 * Circular buffer is used for SDO communication.
 * It can be read or written between successive SDO calls.
 * So size of the buffer can be lower than size of the actual size of data transferred.
 * If only segmented transfer is used, then buffer size can be as low as 7 bytes,
 * if data are read/written each cycle.
 * If block transfer is used, buffer size should be set to at least 1000 bytes, so
